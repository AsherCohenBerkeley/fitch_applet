...
st.write(r"**IMPORTANT NOTE**: Both of these special subproofs require introducing a new constant symbol. **:blue[In this applet, the only symbols which are interpreted as constant symbols instead of variable symbols are '$c$' and '$c_i$' for some integer $i$ (for the latter, you would write c_3 for example in the textbox, just as you would in standard LaTeX). Make sure to only use these symbols when performing $\forall I$ or $\exists E$.]**")
